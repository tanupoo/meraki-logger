...
ap.add_argument("--error-file", action="store", dest="error_file",
                help="specify the error filename.")
...
if opt.log_url:
    handler = HTTPRequestsHandler(opt.log_url, error_file=opt.error_file)
    handler.setLevel(logging.DEBUG)
    logger.addHandler(handler)
